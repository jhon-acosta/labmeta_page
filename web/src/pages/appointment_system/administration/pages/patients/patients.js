...
class Patients extends Component {
  ...
  componentDidMount() {
    axios
      .get(`${API}persona?pers_tip_id=2`)
      .then((response) => {
        this.setState({ patients: response.data.datos });
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  updateData = (
    ...
    this.props.history.push("/admin/updatepatient");
  };
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        Swal.fire({
          ...
          window.location.assign("http://localhost:3000/admin/patients")
        );
      }
    });
  };
        ...
        <div className="flex flex-col ml-48 p-4">
          <div className="py-4">
            <div className="justify-center my-5 select-none flex">
              <p className="mt-5 text-center mr-10 text-2xl">Pacientes.</p>
            </div>
