...
class Updateidentificationtype extends Component {
  constructor(props) {
    super(props);
    this.state = {
      id: localStorage.getItem("id"),
      pers_tip_ide_des: localStorage.getItem("pers_tip_ide_des"),
    };
  }
  ...
  updateData = (e) => {
    ...
    this.update = {
      datos: {
        id: this.state.id,
        pers_tip_ide_des: this.state.pers_tip_ide_des,
      },
    };

    if (
      this.update.datos.id === "" ||
      this.update.datos.pers_tip_ide_des === ""
    ) {
      Swal.fire("", "Complete todos los datos para continuar...!");
    } else {
      axios
        .put(
          `${API}persona_tipo_identificacione?id=${this.state.id}`,
          this.update
        )
        .then((response) => {
  ...
  render() {
    const { pers_tip_ide_des } = this.state;
    return (
      <div>
        <Sidebar />
        <Header />
          ...
          <form className="py-16 px-64 flex items-center">
            ...
            <div className="px-1 flex-1">
              <input
                ...
                type="text"
                name="pers_tip_ide_des"
                value={pers_tip_ide_des}
...
export default withRouter(Updateidentificationtype);
