...
class Updatepeopletype extends Component {
  constructor(props) {
    super(props);
    this.state = {
      id: localStorage.getItem("id"),
      pers_tip_des: localStorage.getItem("pers_tip_des"),
    };
  }
  ...
  updateData = (e) => {
    ...
    if (this.update.datos.id === "" || this.update.datos.pers_tip_des === "") {
      Swal.fire("", "Complete todos los datos para continuar...!");
    } else {
      axios
        .put(`${API}persona_tipo?id=${this.state.id}`, this.update)
        .then((response) => {
...
export default withRouter(Updatepeopletype);
